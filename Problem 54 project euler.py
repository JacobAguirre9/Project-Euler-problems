...
#     Hands                 Player 1            Player 2            Winner
...
# Okay, so let's begin our solution

def handcomputation():
	answer = sum(1 for handpair in HANDS if is_player1_win(handpair))
	return str(answer)

# However, what is the word "handpair" that we have just coded?
# I define handpair to be any given space-separated string of 10 cards.
def is_player1_win(handpair):
	...
	# Now we can compare hand scores
	return get_score(player1) > get_score(player2)

# Now we need to talk about what a "Hand" in poker is. A hand can sort of be seen as an array or vector of cards
# Consider a scenario where we have two players, A & B. What is hand_a were to beat hand_b.
# If this happens, we are only interested in the score for player a, correct?
# So, we only need to capture the score for hand_a. Now, consider if the scores were tied.
# This would be modeled as score_hand_a = score_hand_b

# So, we need to make sure that the code does not rake into effect the consecutiveness of the scores.
#  Comparison is given by the function of get_score().

def get_score(hand):
    assert len(hand) == 5

    # This function will tell us the number of cards within the code with a rank of "i", defined by the user
    rankcounts = [sum(1 for (rank, _) in hand if rank == i) for i in range(13)]

    # This is simply a function to count the number of times this occurs.
    rankcounthist = [rankcounts.count(i) for i in range(6)]

    # flushsuit is in the range bounded in [0,3] if all of these cards contain the same suit; otherwise , we return a -1
...
# Hands is the dictionary given to us by project euler
